...
- Set(key, value, timestamp): store the key with value at timestamp
- Get(key, timestamp): retrieve the value associated with key at a given timestamp

If there is no exact timestamp match, return the value at the most recent timestamp before the given timestamp.

Example:
- Set("foo", "bar", 1)
- Get("foo", 1) -> "bar"
- Get("foo", 3) -> "bar" (most recent before 3)
